...
const KonnectedHelper = require('./konnected-helper');
...
class KonnectedAdapter extends Adapter {
  ...
  constructor(addonManager, manifest) {
    ...
    this.ssdpInterval;
    this.ipAddress = KonnectedHelper.getIpAddress();
    this.port;
    ...
    app.put('/device/:mac', (req, res) => {
      ...
      this.handleDeviceUpdate(mac, pin, state);
      console.log('put mac ', req.body, state, pin);
      res.status(200).contentType('application/json').send(`${req.params.id}`);
    });
    ...
    const server = app.listen(0, () => {
      ...
      db.open().then(() => {
        ...
        return null;
        // TODO Need to probably shut the adapter 
        // down as the API wont be up to date.
      });
    });
  }
  ...
  updateEnabledKonnectedDevices() {
    return new Promise((resolve, reject) => {
      this.config.devices.forEach((device) => {
        ...
        axios({
          method: 'put',
          url: `${device.api_endpoint}/settings`,
          data: data,
          headers: {
  ...
  setKonnectedDevicePin(mac, pin, value) {
    return new Promise((resolve, reject) => {
      ...
      console.log('Setting ', device, pin, value, api_endpoint);
      axios({
        method: 'put',
        url: api_endpoint,
  ...
  handleDeviceUpdate(mac, pin, state) {
    // Here we need to determine the device type and then set the properties
    return new Promise((resolve, reject) => {
      const deviceId = `konnected-${mac}-${pin}`;
      const device = this.devices[deviceId];

      if (device) {
        device.updateDeviceActivity(state);
        resolve();
      } else {
        reject();
      }
