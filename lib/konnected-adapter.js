...
class KonnectedAdapter extends Adapter {
    ...
    return new Promise((resolve, reject) => {
      this.config.devices.forEach((device) => {
        ...
        if (device.enabled) {
          ...
          for (let i = 1; i < 8; i++) {
            const zone = `zone${i}`;
            const konnectedType = device[zone];
            const pin = KonnectedClassifier.mapZoneToPin(i);
  ...
  /**
   * Handle the SSDP Client Notifications
   *
   * TODO
   *
   */
  handleSSDPNotify() {
    console.log('handleSSDPClientOnNotify Got a notification.')
  }
  ...
  handleSSDPResponse(headers, code, rinfo) {
    ...
    console.log('handleSSDPClientOnResponse - Got a response to an m-search:\n%d\n%s\n%s',
      code, JSON.stringify(headers, null, '  '), JSON.stringify(rinfo, null, '  '));
 ...
 */
  ...
  addDevice(deviceId, mac, pin, ip, deviceType, boardType) {
    ...
    console.log('addDevice - ', deviceId);
  ...
  startPairing(_timeoutSeconds) {
    console.log('KonnectedAdapter:', this.name,
   ...
   */
  cancelPairing() {
    console.log('KonnectedAdapter:', this.name, 'id', this.id,
      'pairing cancelled');
    this.pairing = true;
    clearInterval(this.ssdpInterval);
  }
 ...
 * @param {Object} device Device to unpair with
 */
  pairDevice(device, deviceDescription) {
    console.log('KonnectedAdapter:', this.name, 'id', this.id,
      'pariDevice(', device.id, ') started');
  }
